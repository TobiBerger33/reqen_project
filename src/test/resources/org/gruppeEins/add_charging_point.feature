Feature: add charging point
  ...
  Scenario: Add a new charging point to an existing location
    ...
    When I add a charging point with identifier "CP-101" of type "AC" to "City Center"
    Then the charging point "CP-101" is stored in the system
    And its initial state is "in operation free"
    And it is visible under the location "City Center"

  Scenario: Attempt to add a charging point with a duplicate identifier
    Given a location "Airport Station" exists with a charging point "CP-200"
    When I try to add another charging point with identifier "CP-200" to "Airport Station"
  ...
  Scenario: Add charging point to a non-existent location
    When I try to add a charging point with identifier "CP-300" of type "DC" to the location "Unknown Plaza"
    Then the system rejects the request
